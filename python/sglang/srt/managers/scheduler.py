    ...
    def set_internal_state(self, recv_req: SetInternalStateReq):
        ...
        for k, v in server_args_dict.items():
            if k not in args_allow_update:
                ...
                logging.warning(
                    f"Updating {k} to {v!r} is rejected because it is not one of off, calibrate, accept."
                )
                if_success = False
                break
            elif k == "dflash_lossy_spec_threshold" and v < 1.0:
                logging.warning(
                    f"Updating {k} to {v} is rejected because it must be at least 1.0."
                )
                if_success = False
                break
        if if_success and next_mode != "off":
            global_args = get_global_server_args()
            if global_args.speculative_algorithm != "DFLASH":
                logging.warning(
                    "dflash_lossy_spec_mode requires speculative_algorithm=DFLASH."
                )
                if_success = False
            elif global_args.speculative_eagle_topk not in (None, 1):
                logging.warning("dflash_lossy_spec_mode requires topk=1 verification.")
                if_success = False
            elif global_args.disable_overlap_schedule:
                logging.warning(
                    "dflash_lossy_spec_mode requires spec-v2 overlap scheduling."
                )
                if_success = False
        ...
        if if_success:
            ...
            if any(k.startswith("dflash_lossy_spec_") for k in server_args_dict):
                global_args = get_global_server_args()
                global_args.dflash_lossy_spec_score_count = 0
                global_args.dflash_lossy_spec_score_sum = 0.0
                global_args.dflash_lossy_spec_forced_accept_count = 0
                global_args.dflash_lossy_spec_score_histogram = {}
