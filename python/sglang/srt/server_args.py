...
class ServerArgs:
    ...
    dflash_lossy_spec_mode: A[
        Literal["off", "calibrate", "accept"],
        Arg(
            help="DFLASH lossy speculative decoding mode.",
            choices=["off", "calibrate", "accept"],
        ),
    ] = "off"
    dflash_lossy_spec_threshold: A[
        float,
        "DFLASH lossy speculative decoding perplexity threshold. Values must be at least 1.0.",
    ] = 1.0
    dflash_lossy_spec_calibration_output: A[
        Optional[str],
        "Optional JSONL file for DFLASH lossy calibration records.",
    ] = None
    ...
    def check_server_args(self):
        ...
        self.dflash_lossy_spec_score_histogram = {}
        if self.dflash_lossy_spec_threshold < 1.0:
            raise ValueError("dflash_lossy_spec_threshold must be at least 1.0.")
        if self.dflash_lossy_spec_mode != "off":
            if self.speculative_algorithm != "DFLASH":
                raise ValueError(
                    "dflash_lossy_spec_mode requires --speculative-algorithm DFLASH."
                )
            if self.speculative_eagle_topk not in (None, 1):
                raise ValueError("dflash_lossy_spec_mode requires topk=1 verification.")
            if self.disable_overlap_schedule:
                raise ValueError(
                    "dflash_lossy_spec_mode requires spec-v2 overlap scheduling."
                )
            if not is_cuda():
                raise ValueError("dflash_lossy_spec_mode is only supported on CUDA.")
